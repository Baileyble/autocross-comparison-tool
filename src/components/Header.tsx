        ...
        <div className="flex items-center gap-2">
          {hasComparison && !showGarage && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium text-muted hover:text-foreground hover:bg-surface-hover transition-colors btn-tactile"
              >
          ...
          {showGarage && !hasComparison && (
            <div className="text-xs text-muted font-mono">
              {session.runs.length} run{session.runs.length !== 1 ? "s" : ""} loaded
            </div>
          )}
        </div>
      </div>
